...
TEST_CASE("testMakeSmallMaze", "[weight=10][part2]")
{
    ...
    out->writeToFile("testMakeSmallMaze.png");
...
TEST_CASE("testMakeMazeConnected", "[weight=10][part2]")
{
    ...
    maze.makeMaze(15, 15);
    PNG* out = maze.drawMaze();
    out->writeToFile("testMakeMazeConnected.png");
...
TEST_CASE("testMakeMazeAcyclic", "[weight=10][part2]")
{
    ...
    maze.makeMaze(15, 15);
    PNG* out = maze.drawMaze();
    out->writeToFile("testMakeMazeAcyclic.png");
...
TEST_CASE("testSolutionCorrectSquare", "[weight=10][part2]")
{
    ...
    copyMaze(soln, &maze);
    vector<int> solution = maze.solveMaze();
    PNG* out = maze.drawMazeWithSolution();
    out->writeToFile("testSolutionCorrectSquare.png");
    int x = 0;
    int y = 0;
    for (size_t i = 0; i < solution.size(); i++)
        advancePosition(&x, &y, solution[i]);
