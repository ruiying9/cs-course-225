...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    string word1U = word1;
    string word2U = word2;
    transform(word1.begin(), word1.end(), word1U.begin(), ::toupper);
    transform(word2.begin(), word2.end(), word2U.begin(), ::toupper);
    auto iter1 = dict.find(word1U);
    auto iter2 = dict.find(word2U);
    std::vector<std::string> pronun1;
    std::vector<std::string> pronun2;
    if (iter1 != dict.end() && iter2 != dict.end()) {
        pronun1 = iter1->second;
        pronun2 = iter2->second;
    } else {
        return false;
    }
    int check=0;
    if(pronun1.size()==pronun2.size()){
        for(int i=0;i<(int)pronun2.size();i++){
            if(pronun1[i]!=pronun2[i]){
                check=1;
            }
        }
        if(check==0){
